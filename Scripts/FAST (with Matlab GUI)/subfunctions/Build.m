...
handles.HSS_Inertia = varargin{37};
handles.Gearbox_Efficiency = varargin{38};
handles.Gearbox_Ratio = varargin{39};
...
fprintf(fid, '   1        PCMode      - Pitch control mode {0: none, 1: user-defined from routine PitchCntrl, 2: user-defined from Simulink/Labview} (switch)\n');
fprintf(fid, '   0.0      TPCOn       - Time to enable active pitch control (s) [unused when PCMode=0]\n');
fprintf(fid, '   2        VSContrl    - Variable-speed control mode {0: none, 1: simple VS, 2: user-defined from routine UserVSCont, 3: user-defined from Simulink/Labview} (switch)\n');
...
fprintf(fid, ' %5.1f      RotSpeed    - Initial or fixed rotor speed (rpm)\n', handles.Rated_TipSpeedRatio*10/handles.Blade_Radius(end) * 60/(2*pi));
...
fprintf(fid, '  60.0      TStart      - Time to begin tabular output (s)\n');
...
fprintf(fid, '"%s"                         FoilNm      - Names of the airfoil files [NumFoil lines] (quoted strings)\n', [pwd, '\subfunctions\AeroData\Cylinder1.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\Cylinder2.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\DU40_A17.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\DU35_A17.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\DU30_A17.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\DU25_A17.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\DU21_A17.dat']);
fprintf(fid, '"%s"\n', [pwd, '\subfunctions\AeroData\NACA64_A17.dat']);
...
A{17} = '   0        VSContrl    - Variable-speed control mode {0: none, 1: simple VS, 2: user-defined from routine UserVSCont, 3: user-defined from Simulink/Labview} (switch)';
...
fprintf(fid, '   3        TrimCase    - Trim case {1: find nacelle yaw, 2: find generator torque, 3: find collective blade pitch} (switch) [used only when CalcStdy=True and GenDOF=True]\n');
...
for i = length(WindSpeeds)
    ...
    Outputs = [DescOutput];
    States = [DescStates];
    sysm{i} = ss(AvgAMat,[AvgBMat AvgBdMat],AvgCMat,[AvgDMat AvgDdMat],'InputName',Inputs,'Outputname',Outputs);

end

save(get(handles.LinearFile_textbox, 'String'), 'sysm')
disp(' ')
disp(' ')
