...
% to scaling laws.
optimized_turbine.Blade_Mass = old_turbine.Blade_Mass .* chord_ratio .^ 2;
optimized_turbine.Blade_EIflap = old_turbine.Blade_EIflap .* chord_ratio .^ 4;
optimized_turbine.Blade_EIedge = old_turbine.Blade_EIedge .* chord_ratio .^ 4;

% Compute stresses and deflections for the optimized and reference turbine
[optimized_turbine.u_y, optimized_turbine.u_z, optimized_turbine.My, optimized_turbine.Mz] = compute_structural_state( optimized_turbine, optimized_turbine.V_rated );
[reference_turbine.u_y, reference_turbine.u_z, reference_turbine.My, reference_turbine.Mz] = compute_structural_state( reference_turbine, 11.5 );
...
sigma_ratio_y = optimized_turbine.My(1: end - 5) ./ reference_turbine.My(1: end - 5) .* reference_turbine.Blade_EIedge(1: end - 5) ./ optimized_turbine.Blade_EIedge(1: end - 5) .* optimized_turbine.Blade_Chord(1: end - 5) ./ reference_turbine.Blade_Chord(1: end - 5); 
sigma_ratio_z = optimized_turbine.Mz(1: end - 5) ./ reference_turbine.Mz(1: end - 5) .* reference_turbine.Blade_EIflap(1: end - 5) ./ optimized_turbine.Blade_EIflap(1: end - 5) .* optimized_turbine.Blade_Chord(1: end - 5) ./ reference_turbine.Blade_Chord(1: end - 5); 

% Normalized radius for plots
r = optimized_turbine.Blade_Radius(1:end -5) / optimized_turbine.Blade_Radius(end);
...
Initial_Blade_Total_Mass = trapz(optimized_turbine.Blade_Mass) * (optimized_turbine.Blade_Radius(2) - optimized_turbine.Blade_Radius(1)); 
...
Inertia_Adjust = [Inertia_Adjust; ones(length(optimized_turbine.Blade_Radius) - length(Inertia_Adjust), 1) .* Inertia_Adjust(end)];
...
% porportional to A^2.
optimized_turbine.Blade_EIedge = optimized_turbine.Blade_EIedge .* Inertia_Adjust;
optimized_turbine.Blade_EIflap = optimized_turbine.Blade_EIflap .* Inertia_Adjust;
optimized_turbine.Blade_Mass = optimized_turbine.Blade_Mass .* sqrt(Inertia_Adjust);
...
fprintf('Blade mass changed by %f%% \n', (Final_Blade_Total_Mass - Initial_Blade_Total_Mass) /  Initial_Blade_Total_Mass * 100);
